...
/**
 * SearchEngine_Response_Promotion parses and defines a "promotion" in the API response
 *
 ...
 */
class SearchEngine_Response_Promotion extends SearchEngine_Response_DataAbstract
{
    // ------------------------------------------------------
    // Properties
    // ------------------------------------------------------
    ...
    /**
     * @var SearchEngine_Response_Promotion_Image
     */
    protected $image;
    ...
    protected function parse(stdClass $resultData)
    {
        ...
        if (is_array($bodyLines) && isset($bodyLines[0]) && $bodyLines[0] instanceof stdClass)
        {
            foreach($bodyLines as $bodyLine)
            {
                ...
                {
                    array_push($this->bodyLines, new SearchEngine_Response_Promotion_BodyLine($bodyLine));
                }
            }
        }
        ...
        {
            $this->image = new SearchEngine_Response_Promotion_Image($image);
        }
    }
     ...
     *
     * @return SearchEngine_Response_Promotion_Image
     */
