...
/**
 * SearchEngine_Response_Context parses and defines a "context" in the API response
 * 
 * @author Akram Abbasi <mohdakramabbasi@gmail.com>
 * @link https://code.google.com/apis/customsearch/v1/reference.html
 */
class SearchEngine_Response_Context extends SearchEngine_Response_DataAbstract
{
    // ------------------------------------------------------
    // Properties
    // ------------------------------------------------------
    ...
    protected function parse(stdClass $resultData)
    {
        ...
        if (is_array($facets))
        {
            foreach($facets as $facet)
            {
                ...
                {
                    array_push($this->facets, new SearchEngine_Response_Context_Facet($facet[0]));
                }
            }
        }
    }
